...
def loadKeyboardLayout(fName):
    ...
    # targetChar,lines,mean(absoluteX),std(absoluteX),mean(absoluteY),std(absoluteY)
    # Hint: '(', ')' will be removed by the function numpy.genfromtxt 
    dataCSV = np.genfromtxt(fName, dtype = None, delimiter = ',', names = True)

    letters = dataCSV['targetChar']

    posX = dataCSV['meanabsoluteX']
    stdX = dataCSV['stdabsoluteX']

    posY = dataCSV['meanabsoluteY']
    stdY = dataCSV['stdabsoluteY']
